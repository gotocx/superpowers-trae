...
| `Task tool (general-purpose)` | Trae `Task` subagent using the provided prompt; prefer a matching `.trae/agents` named subagent when available |
...
When a skill references a prompt template, read that template and pass the completed prompt to Trae `Task`. Use the matching named subagent from `.trae/agents/` when available, but still include the full task prompt and file paths. Do not ask subagents to infer context from chat history.
